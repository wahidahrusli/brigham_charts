...
class SingleChart extends Component{
	...
	multiFormat(date) {
		...
		this.setState({timeFrame: this.primaryTimeFrameParse(this.dataset)})

    this.setState({start_date: this.timeFrame.start_date})
    this.setState({finish_date: this.timeFrame.finish_date})
		this.setState({measure_values: this.timeFrame.measure_values})
	...
	zoomFunction(){
		...
		let {domain_names, ranges, yDomainMap, new_height} = this.yDomainNames(this.dataset);
		height = new_height;
    ...
    let svgViewport = select("body")
		...
		let xAxisScale =scaleTime()
  			.domain([this.start_date, this.finish_date])
  			.range([0,width]);
    ...
    let innerSpace = svgViewport.append("g")
		...
		let lineDataset = this.lineFuncData(this.timeFrame.measure_values);
		...
		let gX = innerSpace.append("g")
				    ...
				    .call(xAxis.tickFormat(this.multiFormat));
